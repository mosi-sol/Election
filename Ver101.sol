...
contract VotePlatform {
    ...
    uint256 immutable private _endDate = 30; // {60 * 60 * 24} = 86400 = 1 day
    uint256 private _endElectionTime;
    bool private _start = false; // strat + end
    bool private _countVote = false;
    ...
    function finish(address candidator) external view returns (uint256) { // sort by vote
       return _getFinalVoting(candidator);
    }
    
    function totalVotes() internal view returns (uint256) {
